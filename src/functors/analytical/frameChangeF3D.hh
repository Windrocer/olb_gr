...
template <typename T>
Rectangle1DPoiseuille3D<T>::Rectangle1DPoiseuille3D(std::vector<T>& x0_, std::vector<T>& x1_,
    std::vector<T>& x2_, std::vector<T>& maxVelocity_, int periodicDirection_)
  : AnalyticalF3D<T,T>(3), clout(std::cout,"RectanglePoiseille3D"), x0(x0_), x1(x1_),
    x2(x2_), maxVelocity(maxVelocity_) { }


template <typename T>
Rectangle1DPoiseuille3D<T>::Rectangle1DPoiseuille3D(SuperGeometry3D<T>& superGeometry_,
    int material_, std::vector<T>& maxVelocity_, T offset, int periodicDirection_)
  : AnalyticalF3D<T,T>(3), clout(std::cout, "RectanglePoiseuille3D"), maxVelocity(maxVelocity_)
{
  std::vector<T> min = superGeometry_.getStatistics().getMinPhysR(material_);
  std::vector<T> max = superGeometry_.getStatistics().getMaxPhysR(material_);
  ...
  x1 = min;
  x2 = min;
  periodicDirection = periodicDirection_;
...
template <typename T>
bool Rectangle1DPoiseuille3D<T>::operator()(T output[], const T x[])
{
  std::vector<T> velocity(3,T());
...
//  T positionFactor = 16.*d[0]/l[0]*d[1]/l[0]*d[2]/l[1]*d[3]/l[1]; // between 0 and 1
  ...
  if (periodicDirection == 0) {
	positionFactor = 4.*d[0]/l[0]*d[1]/l[0];
  }
  else if (periodicDirection == 1) {
	positionFactor = 4.*d[2]/l[1]*d[3]/l[1];
  }
  output[0] = maxVelocity[0]*positionFactor;
  output[1] = maxVelocity[1]*positionFactor;
  output[2] = maxVelocity[2]*positionFactor;
