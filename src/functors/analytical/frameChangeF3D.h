...
/**
 * This functor returns a Poiseuille profile in one direction with square shaped cross-section.
 * The other direction is uniform --- this functor should be used with slip wall or periodic
 * boundary condition!
 */
/// Velocity profile for pipes with rectangular cross-section.
template<typename T>
class Rectangle1DPoiseuille3D final : public AnalyticalF3D<T,T> {
protected:
	OstreamManager clout;
	std::vector<T> x0;
	std::vector<T> x1;
...
public:
	Rectangle1DPoiseuille3D(std::vector<T>& x0_, std::vector<T>& x1_, std::vector<T>& x2_, std::vector<T>& maxVelocity_, int periodicDirection_);
	/// constructor from material numbers
	/** offset is a positive number denoting the distance from border
	 * voxels of material_ to the zerovelocity boundary */
	Rectangle1DPoiseuille3D(SuperGeometry3D<T>& superGeometry_, int material_, std::vector<T>& maxVelocity_, T offset, int periodicDirection_);
	bool operator()(T output[], const T x[]) override;
};
