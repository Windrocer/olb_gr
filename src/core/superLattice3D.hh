...
// Implement by Zhishang Xu for 3D grid refinement
template<typename T, typename DESCRIPTOR>
void SuperLattice3D<T,DESCRIPTOR>::set(Vector<int,4> pos, const Cell<T,DESCRIPTOR>& cell)
{
#ifdef PARALLEL_MODE_MPI
	if (this->_loadBalancer.isLocal(pos[0])) {
		_blockLattices[this->_loadBalancer.loc(pos[0])].get(pos[1],pos[2],pos[3]) = cell;
	}
#else
	_blockLattices[this->_loadBalancer.loc(pos[0])].get(pos[1],pos[2],pos[3]) = cell;
#endif
...
// Implemented by Zhishang Xu for 3D grid refinement
template<typename T, typename DESCRIPTOR>
void SuperLattice3D<T,DESCRIPTOR>::get(Vector<int,4> pos, Cell<T,DESCRIPTOR>& cell) const
{
#ifdef PARALLEL_MODE_MPI
	...
	if (this->_loadBalancer.isLocal(pos[0])) {
		_blockLattices[this->_loadBalancer.loc(pos[0])].get(pos[1],pos[2],pos[3]).serialize(cellData);
	}
	...
	if(!this->_loadBalancer.isLocal(pos[0])) {
		...
		cell = _blockLattices[this->_loadBalancer.loc(pos[0])].get(pos[1],pos[2],pos[3]);
	}
...
void SuperLattice3D<T, DESCRIPTOR>::collideAndStream()
{
  ...
  // update the neighbour nodes for periodic boundary
  _commStream.send();
  _commStream.receive();
  _commStream.write();
