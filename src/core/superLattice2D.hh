...
template<typename T, typename DESCRIPTOR>
void SuperLattice2D<T, DESCRIPTOR>::defineTwoDSpongeDynamics(
		T lx, T ly, T L, int buffersize, T omega, T rho_target, T u_target[2],
		Spongedynamics<T,DESCRIPTOR>* spongedynamics)
{
	int locX, locY;
	int pointx = static_cast<int>( lx/L );
	int pointy = static_cast<int>( ly/L );
	for (int i=1; i < buffersize; ++i) {
		T delta = (buffersize - i) / static_cast<T>( buffersize );
		for (int px=1; px <= pointx-1; ++px) {
			for (int py=1; py <= pointy-1; ++py) {
				if (px == i || px == pointx-i || py == i || py == pointy-i) {
					for (int iC=0; iC < this->_loadBalancer.size(); ++iC) {
						if (this->_cuboidGeometry.get(this->_loadBalancer.glob(iC)).checkPoint( px*L, py*L, locX, locY, this->_overlap) ) {
//							Spongedynamics<T, DESCRIPTOR> spongedynamics(
//									omega, instances::getBulkMomenta<T, DESCRIPTOR>() );
							FunctorPtr<Dynamics<T,DESCRIPTOR>> spongedynamics_ = std::unique_ptr<Dynamics<T,DESCRIPTOR>>(new Spongedynamics<T,DESCRIPTOR>(omega, instances::getBulkMomenta<T,DESCRIPTOR>()));
							spongedynamics_->setDelta(delta);
							spongedynamics_->setRho_target(rho_target);
							spongedynamics_->setU_target(u_target);
							_extendedBlockLattices[iC].get(locX,locY).defineDynamics(
									spongedynamics_);
						}
					}
				}
