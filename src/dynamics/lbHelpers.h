...
struct lbHelpers {
  ...
  static T spongeCollision(Cell<T,DESCRIPTOR>& cell, T const& rho, 
		  const T u[DESCRIPTOR::d], T const& omega, T const& delta, 
		  T const& rho_target, const T u_target[DESCRIPTOR::d])
  {
	  return lbDynamicsHelpers<T,typename DESCRIPTOR::BaseDescriptor>
		  ::spongeCollision(cell, rho, u, omega, delta, rho_target, u_target);
  }

//  static T spongeCollision(Cell<T,DESCRIPTOR>& cell, T const& rho,
//		  const T u[DESCRIPTOR::d], const T omega[DESCRIPTOR::q], T const& delta,
//		  T const& rho_target, const T u_target[DESCRIPTOR::d])
//  {
//	  const T uSqr = util::normSqr<T,DESCRIPTOR::d>(u);
//	  for (int iPop = 0; iPop < DESCRIPTOR::q; ++iPop) {
//		  cell[iPop] *= (T)1-omega[iPop];
//		  cell[iPop] += omega[iPop] * lbDynamicsHelpers<T,typename DESCRIPTOR::BaseDescriptor>
//			  ::equilibrium(iPop, rho, u, uSqr);
//	  }
//	  return uSqr;
//  }
...
//  static T spongeCollision(CellBase<T,DESCRIPTORBASE>& cell, T const& rho, 
//		  const T u[DESCRIPTORBASE::d], T const& omega, T const& delta, 
//		  T const& rho_target, const T u_target[DESCRIPTORBASE::d])
//  {
//	  const T uSqr = util::normSqr<T,DESCRIPTORBASE::d>(u);
//	  for (int iPop=0; iPop < DESCRIPTORBASE::q; ++iPop) {
//		  cell[iPop] *= (T)1-omega;
