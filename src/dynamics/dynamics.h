...
/// Implementation of the sponge zone collsion step
template<typename T, typename DESCRIPTOR>
class Spongedynamics : public BasicDynamics<T,DESCRIPTOR> {
public:
	/// Constructor
	Spongedynamics(T omega, Momenta<T,DESCRIPTOR>& momenta);
	/// Collision step
	void collide(Cell<T,DESCRIPTOR>& cell, 
			LatticeStatistics<T>& statistics_) override;
	/// Get local relaxation parameter of the dynamics
	T getOmega() const override;
	/// Set local relaxation parameter of the dynammics
	void setOmega(T omega) override;
	/// Get local Delta
	T getDelta() const;
	/// Set local Delta
	void setDelta(T delta);
	/// Get local Rho_target
	T getRho_target() const;
	/// Set local Rho_target
	void setRho_target(T rho_target);
	/// Get local U_target
	T* getU_target();
	/// Set local U_target
	void setU_target(T u_target[DESCRIPTOR::d]);
private:
	T _omega;
	T _delta;
	T _rho_target;
	T _u_target[DESCRIPTOR::d];
};
