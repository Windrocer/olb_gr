...
////////////////////// Class Spongedynamics ///////////////////////
/** \param omega relaxation parameter, related to the dynamic viscosity
 *  \param momenta a Momenta object to know how to compute velocity momenta
 *  \param delta correction coefficient, related to the distance to the boundary
 *  \param rho_target is the desired far flow density
 *  \param u_target is the desired far flow velocity
 */
template<typename T, typename DESCRIPTOR>
Spongedynamics<T,DESCRIPTOR>::Spongedynamics (
		T omega, Momenta<T,DESCRIPTOR>& momenta)
	: BasicDynamics<T,DESCRIPTOR>(momenta),
	_omega(omega)
{
	// To avoid NaN values at first time step,
	// need to specify an initial value for all private variables
	_delta = (T)0.0;
	_rho_target = (T)1.0;
	for (int iPop = 0; iPop < DESCRIPTOR::d; ++iPop)
		_u_target[iPop] = (T)0.0;
}

template<typename T, typename DESCRIPTOR>
void Spongedynamics<T,DESCRIPTOR>::collide (
		Cell<T,DESCRIPTOR>& cell,
		LatticeStatistics<T>& statistics)
{
	T rho, u[DESCRIPTOR::d];
	this->_momenta.computeRhoU(cell, rho, u);
	T uSqr = lbHelpers<T,DESCRIPTOR>::spongeCollision(cell, rho, u,
			_omega, this->_delta, this->_rho_target, this->_u_target);
	statistics.incrementStats(rho, uSqr);
}

template<typename T, typename DESCRIPTOR>
T Spongedynamics<T,DESCRIPTOR>::getOmega() const
{
	return _omega;
}

template<typename T, typename DESCRIPTOR>
void Spongedynamics<T,DESCRIPTOR>::setOmega(T omega)
{
	_omega = omega;
}

template<typename T, typename DESCRIPTOR>
T Spongedynamics<T,DESCRIPTOR>::getDelta() const
{
	return _delta;
}

template<typename T, typename DESCRIPTOR>
void Spongedynamics<T,DESCRIPTOR>::setDelta(T delta)
{
	_delta = delta;
}

template<typename T, typename DESCRIPTOR>
T Spongedynamics<T,DESCRIPTOR>::getRho_target() const
{
	return _rho_target;
}

template<typename T, typename DESCRIPTOR>
void Spongedynamics<T,DESCRIPTOR>::setRho_target(T rho_target)
{
	_rho_target = rho_target;
}

template<typename T, typename DESCRIPTOR>
T* Spongedynamics<T,DESCRIPTOR>::getU_target()
{
	return _u_target[DESCRIPTOR::d];
}

template<typename T, typename DESCRIPTOR>
void Spongedynamics<T,DESCRIPTOR>::setU_target(T u_target[DESCRIPTOR::d])
{
	for (int iPop = 0; iPop < DESCRIPTOR::d; ++iPop)
		_u_target[iPop] = *(u_target + iPop);
}
