...
// Implemented by Zhishang Xu for 3D grid refinement 2019.10.09
template<typename T>
void SuperGeometry3D<T>::reset(IndicatorF3D<T>& domain)
{
	this->communicate();
	for (unsigned iC = 0; iC < _extendedBlockGeometries.size(); ++iC) {
//		_blockGeometries[iC].reset(domain);
		// If the boundary type is periodic, some overlapped nodes
		// are not reset in using _blockGeometries[iC].reset().
		_extendedBlockGeometries[iC].BlockGeometryStructure3D<T>::reset(domain);
	}
	_statistics.getStatisticsStatus() = true;
	this->_communicationNeeded = true;
}
