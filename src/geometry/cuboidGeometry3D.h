...
public:
  ...
  /// Constructs a cuboid structure with a uniform spacing of voxelSize which consists of nC cuboids, the cuboids not needed are removed and too big ones are shrinked. Assign periodicity on initialization
  CuboidGeometry3D(IndicatorF3D<T>& indicatorF, T voxelSize,  
		  bool periodicityX, bool periodicityY, bool periodicityZ, int nC=1);
  ...
  bool getLatticeR(const Vector<T,3>& physR, Vector<int,4>& latticeR) const;
